...
from ecommerce_agent.settings.media_models import (
    IMAGE_API_URL,
    IMAGE_BACKEND,
    IMAGE_MODEL,
    VIDEO_BACKEND,
...
class MediaSettings:
    ...
    def from_env(cls) -> "MediaSettings":
        output_dir = Path(get_env("MEDIA_OUTPUT_DIR", "agent-app/runtime"))
        return cls(
            image_backend=get_env("IMAGE_BACKEND", IMAGE_BACKEND).lower(),
            seedream_api_url=get_env(
                "SEEDREAM_API_URL",
                IMAGE_API_URL,
            ),
            seedream_api_key=get_env("SEEDREAM_API_KEY"),
            seedream_model=get_env("SEEDREAM_MODEL", IMAGE_MODEL),
            image_size=get_env("IMAGE_SIZE", "landscape_16_9"),
            video_backend=get_env("VIDEO_BACKEND", VIDEO_BACKEND).lower(),
            ...
            seedance_model=get_env("SEEDANCE_MODEL", VIDEO_MODEL),
